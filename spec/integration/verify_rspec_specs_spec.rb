...
RSpec.describe 'RSpec specs:' do
  context 'Core' do
    ...
    it 'run correctly' do
      ...
      # p parsed_results.keys
      # p parsed_results['messages']
      # puts '------------------------------------------'
      # puts results.output.gsub(/(\A|\n)/, '\1> ')
      expect(parsed_results['summary_line']).to include(" 0 failures")
      expect(parsed_results['summary_line']).to eq("337 examples, 0 failures, 35 pending")
